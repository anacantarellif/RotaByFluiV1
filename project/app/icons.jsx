// icons.jsx — line icon set + Flui seal glyph. Exports: Icon, Seal, SeloBadge
...
const P = {
  ...
  settings: 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM19.4 13a7.8 7.8 0 0 0 0-2l2-1.5-2-3.4-2.3 1a7.6 7.6 0 0 0-1.7-1l-.3-2.6h-4l-.3 2.6a7.6 7.6 0 0 0-1.7 1l-2.3-1-2 3.4L4.6 11a7.8 7.8 0 0 0 0 2l-2 1.5 2 3.4 2.3-1a7.6 7.6 0 0 0 1.7 1l.3 2.6h4l.3-2.6a7.6 7.6 0 0 0 1.7-1l2.3 1 2-3.4-2-1.5Z',
...
function Icon({ name, size = 22, stroke = 2, fill = 'none', color = 'currentColor', style, label }) {
  const a11y = label ? { role: 'img', 'aria-label': label } : { 'aria-hidden': true, focusable: 'false' };
  const art = FILLED[name];
  if (art) {
    return React.createElement('svg', Object.assign({
      width: size, height: size, viewBox: art.vb, fill: 'none', style,
    }, a11y), React.createElement('path', { d: art.d, fill: color }));
  }
  const d = P[name];
  return React.createElement('svg', Object.assign({
  ...
  }, a11y), React.createElement('path', { d }));
}

// Flui seal — award-star badge (supplied artwork)
function Seal({ size = 16, filled = true, label }) {
  const c = 'currentColor';
  const a11y = label ? { role: 'img', 'aria-label': label } : { 'aria-hidden': true, focusable: 'false' };
  return React.createElement('svg', Object.assign({ width: size, height: size, viewBox: '0 0 27 27', fill: 'none' }, a11y),
    React.createElement('path', {
      ...
      fill: filled ? c : 'none', stroke: filled ? 'none' : c, strokeWidth: filled ? 0 : 1.6,
    }),
    React.createElement('path', {
      d: 'M12.9012 14.7002V19.0752L17.3248 11.7836H14.1651V7.40857L9.74146 14.7002H12.9012Z',
      fill: filled ? 'var(--seal-bolt, #fff)' : c,
    })
  );
}

function SeloBadge({ level = 1, label }) {
  const txt = label || (level >= 3 ? 'Selo Flui · Excelência' : level === 2 ? 'Selo Flui · Destaque' : 'Selo Flui');
  return React.createElement('span', { className: 'selo-badge' },
    React.createElement('span', { className: 'seal-row' },
      Array.from({ length: level }).map((_, i) => React.createElement('span', { key: i, className: 'seal', style: { width: 15, height: 15 } }, React.createElement(Seal, { size: 15 })))
    ),
    txt
  );
}

Object.assign(window, { Icon, Seal, SeloBadge });
