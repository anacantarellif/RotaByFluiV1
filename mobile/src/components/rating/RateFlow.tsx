    ...
    >
      <View style={{ flex: 1 }}>
        ...
        {/* ---------- scrollable body ----------
            ...
            nothing to scroll). Plain RN ScrollView (not BottomSheetScrollView)
            gives it somewhere to go — same fix, same reasoning, as
            StationDetailContent's ficha: BottomSheetScrollView's pan gesture
            runs through react-native-gesture-handler, which the map's WebView
            (GeoMapView.tsx → LeafletMapView.tsx, also on MapScreen where this
            flow opens) can leave corrupted for the rest of the screen even
            after unmounting; plain ScrollView uses RN's native
            ScrollResponder instead, unaffected by that. */}
